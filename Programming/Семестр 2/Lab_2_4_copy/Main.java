...
Напишите класс FinanceManager который будет содержать помимо массива транзакций методы для добавления транзакций в конец массива, 
нахождения сумм доходов с использованием ссылок на методы, упорядочивания транзакций по различным 
переменным с  использованием ссылок на методы, а также фильтрации по различным критериям с использованием лямбда-выражений.
...
Упорядочивание должно выполняться без создания объекта FinanceManager методом простого обмена 
по неубыванию и по невозрастанию по суммам и по датам сначала доходы потом расходы. 
При фильтрации создаются новые объекты FinanceManager , критерии отбора: доход или расход, 
расход и доход вместе позже заданной даты.
*/
public class Main {
    public static void main(String[] args) {
        ...
        // Фильтрация транзакций: доходы после определенной даты
        ...
        System.out.println("\nФильтрация расходов после 2024-07-02:");
        FinanceManager filteredManager2 = manager
                .filter(t -> "расход".equals(t.getType()) && t.getDate().isAfter(LocalDate.of(2024, 7, 2)));
        filteredManager2.printTransactions();
